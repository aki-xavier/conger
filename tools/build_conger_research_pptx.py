#!/usr/bin/env python3
"""可重复生成 conger 研究汇报 PPTX（第一人称研究复盘口吻）。
...
def s1_title(prs):
    ...
    tx(s, 0.9, 1.95, 11.5, 0.4, [[run("研究路线复盘 · 第一人称 · 2026-08", 13, True, TEAL)]])
    tx(s, 0.9, 2.45, 11.6, 1.9, [
        [run("我是如何从 Riesz-GMM 显式建模", 38, True, DARK)],
        [run("切换到实例级 MixtureSPN 逆渲染", 38, True, DARK)],
    ], space_after=6)
    hline(s, 0.9, 4.35, 2.2, TEAL, 2.0)
    tx(s, 0.9, 4.6, 11.0, 0.9, [
        [run("一次由实测否决驱动的转向复盘:", 17, False, GRAY)],
...
def s2_overview(prs):
    ...
    tx(s, 7.75, 1.95, 4.9, 0.4, [[run("新: 我把显式放在生成过程里", 18, True, DARK)]])
    ...
    # 底部: 三阶段时间轴
    tx(s, 0.6, 5.0, 6.0, 0.32, [[run("我的路线时间轴", 12, True, GRAY)]])
    yl = 5.4
...
def s3_problem(prs):
    ...
    # 输出自由度
    tx(s, 0.7, 3.75, 6.0, 0.32, [[run("输出自由度 × 8(单图元)", 12, True, GRAY)]])
    dofs = ["kind 图元", "u 位置", "v 位置", "s 尺寸", "z 深度", "hue 色相", "lcol 光色", "ldir 光向"]
    x = 0.7
    for d in dofs:
        chip(s, x, 4.12, 1.45, 0.42, d, DARK, size=11)
        x += 1.59
    # 先验形式
...
def s5_old_pipeline(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "旧路线: 我当初显式建模了什么", kicker="旧路线",
          evidence="aaa43e0 (Gabor-GMM) · 3a344b4 (Riesz 引入)")
    ...
    for i, m in enumerate(mods):
        ...
        x += w + 0.185
    tx(s, 0.7, 3.15, 12.0, 0.34, [[run("两阶段显式管线", 15, True, DARK)]])
    tx(s, 0.7, 3.6, 12.0, 1.1, [
        [run("① 07-27 (aaa43e0): ", 15, True, DARK),
         run("Gabor-GMM 软聚类 (DGGG / DG-MRF)", 15, False, DARK)],
        [run("② 08-02 (3a344b4): ", 15, True, DARK),
         run("Riesz 替代 Gabor → Riesz + VB-GMM + CGA 显式管线", 15, False, DARK)],
        [run("逐模块闭式、手工先验/门控、参数 β·T; 无训练数据、无梯度优化", 14.5, False, DARK)],
    ], space_after=5)
    ...
    commits = [
        ...
        (2.9, "aaa43e0", "07-27 废弃报告(Gabor-GMM/DG-MRF)", RED),
        (5.2, "b52ac7a", "07-31 CGA implementation", DARK),
        (7.3, "3a344b4", "08-02 Riesz 引入", DARK),
    ...
    footer(s, 5, "aaa43e0 (Gabor) · 3a344b4 (Riesz) · cbc9fa8 · 43416c8")
    return s


def s6_kept(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "我先保留了什么（跨阶段合成）", kicker="旧路线",
          evidence="aaa43e0 (Gabor) + 3a344b4 (Riesz) + current docs")
    # 组 A: 07-27 报告明确保留
    tx(s, 0.7, 1.6, 9.0, 0.3, [[run("07-27 报告明确保留（Gabor-GMM 时代）", 12.5, True, GOLD)]])
    group_a = [
        ("Pb 半盘检测器", "跨候选边界纹理分布"),
        ("增益控制", "除法归一化, 光照不变"),
        ("2θ 圆统计", "方向用圆分布而非高斯"),
    ...
    for i, (t, sub) in enumerate(group_a):
        col = i % 3
        row = i // 3
        gx = 0.7 + col * 4.15
        gy = 1.95 + row * 1.18
        shape(s, gx, gy, 3.85, 0.98, GOLD_SOFT, line=GOLD, line_w=1.0,
              kind=MSO_SHAPE.ROUNDED_RECTANGLE, radius=0.16)
        tx(s, gx + 0.15, gy + 0.08, 3.55, 0.32, [[run(t, 13.5, True, DARK)]])
        tx(s, gx + 0.15, gy + 0.44, 3.55, 0.4, [[run(sub, 10.5, False, GRAY)]])
    # 组 B: 后续证明继续有效
    tx(s, 0.7, 4.5, 9.0, 0.3, [[run("后续证明继续有效", 12.5, True, TEAL)]])
    group_b = [
        ("Riesz 前端", "08-02 3a344b4 引入, 替代 Gabor"),
        ("CGA 场景表示", "图元场景图"),
        ("双目几何 / 前景权重", "立体锚点"),
    ]
    for i, (t, sub) in enumerate(group_b):
        gx = 0.7 + i * 4.15
        shape(s, gx, 4.85, 3.85, 0.98, TEAL_SOFT, line=TEAL, line_w=1.0,
              kind=MSO_SHAPE.ROUNDED_RECTANGLE, radius=0.16)
        tx(s, gx + 0.15, 4.93, 3.55, 0.32, [[run(t, 13.5, True, DARK)]])
        tx(s, gx + 0.15, 5.29, 3.55, 0.4, [[run(sub, 10.5, False, GRAY)]])
    tx(s, 0.7, 6.1, 12.0, 0.6, [
        [run("我的判断:  ", 15, True, GOLD),
         run("失败的只是「聚类 + 手工先验」的显式推理层; 前端谱表示(Gabor→Riesz)与几何表示是正确资产。", 15, False, DARK)],
    ], space_after=3)
    footer(s, 6, "aaa43e0 (07-27) · 3a344b4 (08-02) · docs/architecture.md §2.1")
    return s


def s7_fail1(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "失败证据 I: 我怎么判断这不是参数问题", kicker="旧路线否决",
    ...
    tx(s, rx + 0.25, 2.02, 4.7, 0.4, [[run("MRF 修精度, 但靠三参数标定", 14.5, True, RED)]])
    tx(s, rx + 0.25, 2.5, 4.75, 2.0, [
        [run("β", 13, True, DARK), run(" 邻域一致 = 多少 nat → 逐场景网格搜索", 12, False, DARK)],
        [run("T", 13, True, DARK), run(" softmax 温度 → 二分 ~100–200 步 × 6 轮", 12, False, DARK)],
        [run("门控", 13, True, DARK), run(" Pb>0.5 两步后处理, 0.49/0.51 天差地别", 12, False, DARK)],
    ], space_after=4)
    tx(s, rx + 0.25, 4.85, 4.75, 1.15, [
        [run("12 路改进全否决 ", 12.5, True, RED),
         run("(HMRF-EM 61–66% · pyramid 87.2→48.5 · 显著性 53%)", 11, False, DARK)],
        [run("我的判断: 不是参数没调好, 是手工先验本身不可标定。", 13, True, DARK)],
    ], space_after=3)
...
def s8_fail2(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "失败证据 II: 我为什么转向(拓扑白卷)", kicker="旧路线否决",
    ...
    tx(s, 0.7, 6.75, 12.0, 0.4, [
        [run("我的判断: 手工先验/显式推理层产出与下游脱节 —— 这就是我转向「生成过程显式」的直接动因。", 13.5, True, DARK)],
    ])
...
def s9_pivot(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "我的决定性实验: SPN 场景码 0.970", kicker="SPN 分支验证",
    ...
    tx(s, 1.1, 4.32, 5.0, 0.8, [
        [run("N=2000 · 288 码 = kind3 × 8×6 网格 × 2 尺寸", 12, False, WHITE)],
        [run("枚举后验 argmax", 12, False, WHITE)],
    ], space_after=2)
    ...
    tx(s, 6.7, 3.75, 6.0, 1.6, [
        [run("我看到:  ", 14.5, True, DARK),
         run("「渲染 → 特征 → 生成模型后验」能端到端反推场景因子。", 14.5, False, DARK)],
        [run("于是我认定: 生成模型后验方向可行, SPN 成为主线候选。", 14.5, False, DARK)],
    ], space_after=5)
...
def s10_retire1(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "我为什么退役离散码", kicker="两次退役之一",
    ...
    tx(s, 6.95, 2.1, 5.5, 0.4, [[run("我的理由", 14.5, True, RED)]])
    ...
    tx(s, 0.7, 5.6, 12.0, 0.9, [
        [run("后续:  ", 14, True, DARK),
         run("tied 方差/收缩已在连续化初版稳定似然; 实例级则在下一步「退役质心」时登场。", 14, False, DARK)],
    ], space_after=3)
...
def s11_retire2(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "我为什么退役 EM / 质心", kicker="两次退役之二",
    ...
    tx(s, 7.45, 3.4, 5.0, 0.5, [[run("全量 1296: u/v R²≈0.93 · RMSE 5.4/5.2px", 12, False, DARK)]])
    tx(s, 0.7, 4.35, 6.0, 0.32, [[run("EM 五条退化通道 (§2.2)", 12.5, True, GRAY)]])
    chans = [
        ("目标 razor 门控", "winner-take-all E 步"),
        ("死分量", "均值爆炸 90×"),
        ("方差无上限", "只活 19/216"),
        ("权重死亡", "log_w ≈ −20"),
        ("nk≈3 方差噪声", "±115 nats 淹没距离"),
    ]
    for i, (t, d) in enumerate(chans):
        gx = 0.7 + i * 2.44
        shape(s, gx, 4.72, 2.34, 1.35, LIGHT, radius=0.1, kind=MSO_SHAPE.ROUNDED_RECTANGLE)
        tx(s, gx + 0.12, 4.85, 2.1, 0.32, [[run(t, 12, True, DARK)]])
        tx(s, gx + 0.12, 5.22, 2.1, 0.75, [[run(d, 10.5, False, GRAY)]])
    tx(s, 0.7, 6.3, 12.0, 0.6, [
        [run("我的教训:  ", 14.5, True, GOLD),
         run("EM/质心压缩是大数据优化; 数据均匀采样 ⟹ 均匀权重才是正确先验(学权重反而死亡螺旋)。", 14.5, False, DARK)],
    ], space_after=3)
    footer(s, 11, "54cd2d1 · §2.2 (u≈0.90; 全量 u/v≈0.93)")
    return s


def s12_how(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "我现在把显式性放在哪里", kicker="当前架构",
    ...
    shape(s, 0.7, 5.42, 12.0, 1.4, LIGHT, radius=0.05, kind=MSO_SHAPE.ROUNDED_RECTANGLE)
    tx(s, 0.95, 5.54, 11.5, 1.2, [
        [run("我的显式 = 生成过程 + 结构契约;  其余(统计规律·歧义)全进后验。", 13.5, True, DARK)],
        [run("预测语义: 连续条件期望 E[u,v,s−ŝ,z−ẑ|x] ≡ 分层核回归; 离散因子 P(kind,hue,lcol,ldir|x) = 后验分类。", 12.5, False, DARK)],
        [run("白化使对角高斯 ≡ 原空间全协方差 —— 解决「相邻像素强相关 → 对角高斯重复计票」。", 12, False, GRAY)],
    ], space_after=3)
...
def s13_bench(prs):
    ...
    shape(s, 0.7, 1.8, 6.0, 2.8, LIGHT, radius=0.05, kind=MSO_SHAPE.ROUNDED_RECTANGLE)
    tx(s, 0.95, 1.92, 5.5, 0.32, [[run("插值", 15, True, DARK)]])
    rows_i = [("u / v  R²", "0.930 / 0.945"), ("s  R²", "0.508"), ("z  R²", "0.831"),
              ("kind / hue", "0.753 / 1.000"), ("lcol / ldir", "0.994 / 0.895")]
    _metric_rows(s, rows_i, 1.0, 2.32, 0.44)
    shape(s, 7.0, 1.8, 5.7, 2.8, LIGHT, radius=0.05, kind=MSO_SHAPE.ROUNDED_RECTANGLE)
    tx(s, 7.25, 1.92, 5.2, 0.32, [[run("外推", 15, True, DARK)]])
    rows_e = [("u / v  R²", "0.949 / 0.953"), ("s / z  R²", "0.922 / 0.956"), ("kind", "0.617"),
              ("hue", "0.981"), ("lcol / ldir", "0.880 / 0.772")]
    _metric_rows(s, rows_e, 7.3, 2.32, 0.44)
    shape(s, 0.7, 4.78, 12.0, 0.72, TEAL_SOFT, radius=0.08, kind=MSO_SHAPE.ROUNDED_RECTANGLE)
    tx(s, 0.95, 4.78, 11.5, 0.72, [
        [run("渲染残差精炼:  ", 13, True, TEAL),
         run("lcol/ldir  0.457/0.367", 13.5, True, RED),
         run("  →  ", 13.5, True, GRAY),
         run("0.994/0.895", 13.5, True, TEAL),
         run("   候选重渲染交回反照率×光照歧义", 12, False, GRAY)],
    ], anchor=MSO_ANCHOR.MIDDLE, space_after=0)
    _fit_image(s, ART_SCATTER, 1.0, 5.72, 1.3, 0.95)
    tx(s, 2.35, 5.8, 4.2, 0.9, [
        [run("外推 / 插值 u,v 散点", 12, True, DARK)],
        [run("inverse_scatter.png", 10, False, GRAY)],
    ], anchor=MSO_ANCHOR.MIDDLE, space_after=3)
    _fit_image(s, ART_RECON, 7.4, 5.72, 1.0, 0.95)
    tx(s, 8.2, 5.8, 4.3, 0.9, [
        [run("重建样例 cga.Scene", 12, True, DARK)],
        [run("inverse_recon.png", 10, False, GRAY)],
    ], anchor=MSO_ANCHOR.MIDDLE, space_after=3)
    footer(s, 13, "docs/architecture.md §1 · 精炼前 lcol/ldir 0.457/0.367 → 0.994/0.895")
    return s


def _metric_rows(s, rows, x, y, step=0.52):
...
def s14_echo(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "方法论回声: 我再次验证纹理 / roughness", kicker="近期研究",
    ...
    tx(s, 7.25, 2.4, 5.3, 1.9, [
        [run("纹理类型 ", 13, True, DARK), run("0.50 / 0.46 > chance 0.33", 13, True, TEAL)],
    ...
    tx(s, 0.95, 4.9, 11.5, 1.5, [
        [run("我再次验证:  ", 14.5, True, GOLD),
         run("「实测驱动、诚实否决」—— 固定几何探针的乐观(tex 1.00 / roughness 0.997)被主线全变实测修正(0.50 / 负 R²)。", 14.5, False, DARK)],
...
def s15_summary(prs):
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "复盘: 我带走的三条原则与下一步", kicker="结论",
          evidence="docs/architecture.md §7 · §0")
    prins = [
        ("失败要可证伪", "每个机制给出可测反例, 否决靠数据不靠直觉", RED),
        ("保留有效先验", "信号前端/几何表示留下, 只废掉手工推理层", GOLD),
        ("不确定性统一进后验", "硬约束进支持集 · 歧义进候选后验, 不做过早 argmax", TEAL),
    ]
    ...
    tx(s, 0.95, 4.2, 11.5, 1.1, [
        [run("我的复盘: 显式建模的正确粒度 = 生成过程, 而非推理模块。", 16, True, WHITE)],
