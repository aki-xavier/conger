...
实测域界 (2026-08-10, iBims/NYU 室内评估): 室内 λ̂ 变化是表面驱动
(多表面纹理不同), 非深度驱动 —— 像素级 Spearman −0.006/−0.101,
块级 −0.07/+0.16, 均无单调性。**曾试自动弃权门** (λ̂ 梯度相干度 /
肥尾度判别量): 判别实验失败 —— 相干度室内也 100% (边界产生对齐
梯度), 肥尾度在"含边界的多表面场景"(合成测试场景 p90/med=3.08)
与室内 (3.37) 无法分离, 且会误弃权合成域。**域界检查本身是纹理-
深度消歧难题, 不是廉价统计能做的 → 判负回退**。结论: 单目纹理
尺度只是冷启动线索 (合成域/自然纹理域有效), 度量深度由运动视差
通道承担 (prior.md 双通道设计意图 → 约束); 室内失效已量化记档,
由评估脚本 (demo_ibims_eval/demo_nyu_eval) 持续度量。
