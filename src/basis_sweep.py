...
2916 帧上 ~12min, 跳过), 全维基线见 docs §4。注意: 子集取前 N 帧有组合
偏置, 只用于看趋势; 权威结论以全量 `inverse.py` 验收为准 (见 §10.6)。
"""
...
MODELS = {
    "layered": (
        "artifacts/spn_layered_anchor_mix_144x144_luloluphluorchlochphchor"
        "chlochphchorchrachra_k3h6c3d3o2sv3rp2_sl8.safetensors"
    ),
    "composite": (
        "artifacts/spn_composite_mix_144x144_luloluphluorchlochphchorch"
        "lochphchorchrachra_k3h6c3d3o2sv1rp2_cp2.safetensors"
    ),
}
...
def _eval(p_gt: mx.array, t_pred: mx.array, scene_pred, p_train: mx.array):
    ...
    acc = {
        nm: float(mx.mean((pred[:, j] == p_gt[:, j]).astype(mx.float32)))
        for nm, j in LAYERED_FACTORS
    }
...
def main() -> None:
    ...
    recon = (
        CompositeReconstructor
        if args.family == "composite"
        else LayeredReconstructor
    )
    ...
    print(
        f"{args.family}: full D={full.f_mu.shape[1]} "
        f"K={full.f_mu.shape[0]} subset={n}"
    )
