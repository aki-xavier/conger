...
def disc_scene(gx: int, gy: int) -> Scene:
    """探针 B: 紫色圆盘 (码簿外新类别), 位置投影同 code_to_scene。"""
    u = (gx + 0.5) * W / N_GX
    v = (gy + 0.5) * H / N_GY
    zc = CAM_Z - 3.0
    x = (u - (W - 1) / 2.0) * zc / FX
    y = ((H - 1) / 2.0 - v) * zc / FY
    scene = Scene(background=Color(0x141414))
    scene.add(AmbientLight(Color(0xFFFFFF), 0.5))
    scene.add(DirectionalLight(Color(0xFFFFFF), 0.7, direction=(0.3, -0.7, 0.4)))
    scene.add(
        Mesh(
            CircleGeometry(0.5),
            MeshStandardMaterial(Color(0x8E44AD), roughness=0.55),
            position=(x, y, 3.0),
        )
    )
    return scene
...
def feats_of_discs(
    cells: list[tuple[int, int]], renderer, cam, rw: RieszWavelet
) -> tuple[mx.array, mx.array]:
    full, pooled = [], []
    for gx, gy in cells:
        frame = renderer.render(disc_scene(gx, gy), cam)
        rw.update(frame_lum(frame))
        f = rw.features()
        v = mx.concatenate(
            [f.log_mag.reshape(-1), f.phase_coh.reshape(-1), f.ori_R.reshape(-1)]
...
def build() -> dict[str, mx.array]:
    cache = Path(__file__).resolve().parent.parent / "artifacts"
    cache.mkdir(exist_ok=True)
    path = cache / f"joint_{N_TRAIN}_{N_STREAM}_{N_EVAL}_{N_PROBE}.npz"
    if path.exists():
    ...
    # 流: 70% 已知码 / 30% 未见码, 交错
    u = mx.random.uniform(shape=(N_STREAM,), key=mx.random.key(55)).tolist()
    st = [
        sample_codes(unseen_codes, 1, 55000 + i)[0]
        if r < NOVEL_RATE
        else sample_codes(train_codes, 1, 77000 + i)[0]
        for i, r in enumerate(u)
    ]
    es = sample_codes(train_codes, N_EVAL, 99)
    eu = sample_codes(unseen_codes, N_EVAL, 77)
    cells = [
        tuple(int(v) for v in row)
        for row in mx.random.randint(
            0, 8, shape=(N_PROBE, 2), key=mx.random.key(66)
        ).tolist()
    ]
    cells = [(gx % N_GX, gy % N_GY) for gx, gy in cells]
    d: dict[str, mx.array] = {}
    d["xf1"], d["xp1"] = feats_of(tr, renderer, cam, rw)
    d["c1"] = mx.array(tr, dtype=mx.float32)
    d["xfs"], d["xps"] = feats_of(st, renderer, cam, rw)
    d["cs"] = mx.array(st, dtype=mx.float32)
    d["xes"], d["xpes"] = feats_of(es, renderer, cam, rw)
    d["ces"] = mx.array(es, dtype=mx.float32)
    d["xeu"], d["xpeu"] = feats_of(eu, renderer, cam, rw)
    d["ceu"] = mx.array(eu, dtype=mx.float32)
    d["xpb"], d["xppb"] = feats_of_discs(cells, renderer, cam, rw)
...
t0 = time.monotonic()
cb = CodeBayes.fit(d["xf1"], mx.array(c1, dtype=mx.int32), cards=CARDS)
xz, _, mu_z, sd_z = standardize(d["xp1"], d["xp1"][:1])
code_arr = mx.array([list(idx_to_code(c)) for c in c1], dtype=mx.float32)
xj = mx.concatenate([xz, code_arr], axis=1)
n_feat = d["xp1"].shape[1]
card = dict(zip(range(n_feat, n_feat + 5), CARDS))
spn = learn_spn(xj, disc_cols=set(card), card=card, min_n=3, max_depth=14)
...
def spn_pred(xp_row: mx.array) -> tuple[int, ...]:
    ...
    p = spn.posterior((xp_row - mu_z) / sd_z, codes)
    mx.eval(p)
    return idx_to_code(int(mx.argmax(p[0])))


# ── phase 2: 混合流 (门控 → 快/慢轨 → 提升) ───────────────────────
prov_true: dict[int, list[int]] = {}  # 临时分量 → 吸收帧真值码 (评估用)
fast_hit = fast_n = 0
...
for i in range(N_STREAM):
    ...
    if bool(novel[0]):
        ...
        gt = idx_to_code(true)
        slow_var["kind"] += pt[0] == gt[0]
        slow_var["gx"] += pt[1] == gt[1]
        slow_var["gy"] += pt[2] == gt[2]
        slow_n += 1
        idx = cb.grow()  # 提升: 新内容进码簿
        cb.absorb(xf, mx.array([idx], dtype=mx.int32))
        prov_true.setdefault(idx, []).append(true)
    else:
        post = cb.posterior_all(xf)
        mx.eval(post)
        idx = int(mx.argmax(post[0]))
        cb.absorb(xf, mx.array([idx], dtype=mx.int32))  # 自标注吸收
        if idx >= N_CODES:
            ...
            fast_n += 1
print(
    f"phase2: 流 {N_STREAM} 帧 ({time.monotonic()-t0:.0f}s) | "
    f"快轨 {fast_n} 帧 acc {fast_hit/max(fast_n,1):.3f} | "
    f"慢轨 {slow_n} 帧 kind {slow_var['kind']/max(slow_n,1):.3f} "
    f"gx {slow_var['gx']/max(slow_n,1):.3f} gy {slow_var['gy']/max(slow_n,1):.3f} | "
    f"提升 {len(prov_true)} 分量"
)


def resolve(idx: int) -> int:
    """分量下标 → 码下标: 码簿内恒等; 临时分量用真值多数票 (预言机, 仅评估)。"""
    if idx < N_CODES:
        return idx
...
for i in range(N_PROBE):
    _, novel = cb.gate(d["xpb"][i : i + 1])
    pb_novel += bool(novel[0])
    pt = spn_pred(d["xppb"][i : i + 1])
    pb_var["gx"] += pt[1] == cpb[i][0]
    pb_var["gy"] += pt[2] == cpb[i][1]

print(f"\nphase3 seen : 误判新 {es_novel/N_EVAL:.3f} | 码 acc {es_hit/N_EVAL:.3f}")
print(
    f"phase3 unseen: 仍判新 {eu_novel/N_EVAL:.3f} (SPN 变量级: "
    f"kind {eu_var['kind']/max(eu_novel,1):.3f} gx {eu_var['gx']/max(eu_novel,1):.3f} "
    f"gy {eu_var['gy']/max(eu_novel,1):.3f}) | 已覆盖 {eu_cov/N_EVAL:.3f} "
    f"码 acc {eu_hit/max(eu_cov,1):.3f}"
)
pur = []
...
print(
    f"提升分量: {len(prov_true)} 个, 平均纯度 {sum(pur)/max(len(pur),1):.3f} "
    f"(多数真值码占比)"
)
print(
    f"probe B (圆盘): 判新 {pb_novel/N_PROBE:.3f} | "
    f"SPN 位置 gx {pb_var['gx']/N_PROBE:.3f} gy {pb_var['gy']/N_PROBE:.3f}"
)

# ── 标定断言 (2026-08-12 实测标定, 留余量) ────────────────────────
# seen 误判新 0.060 不是门控失误: 1037 个 seen 码在 2000+420 帧里
# 覆盖率 ≈1−e^{−2.3}≈0.90, 未采到的码被判新是正确行为
assert es_novel / N_EVAL < 0.10, "已知码误判新超过未覆盖码份额"
assert es_hit / N_EVAL > 0.90, f"seen 码 acc 过低 {es_hit/N_EVAL:.3f}"
assert eu_hit / max(eu_cov, 1) > 0.80, "已提升码 acc 过低"
assert pb_novel / N_PROBE > 0.95, "新类别未被门控捕获"
