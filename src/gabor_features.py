...
def _grid(H: int, W: int) -> tuple[mx.array, mx.array, mx.array, int, int, int]:
    yn = list(range(H))
    xn = list(range(W))
    Ny, Nx = len(yn), len(xn)
    sy = mx.array([y for y in yn for _ in xn], dtype=mx.int32)
    sx = mx.array([x for _ in yn for x in xn], dtype=mx.int32)
    px = sx.astype(mx.float32).reshape(-1, 1) / max(W - 1, 1)
    py = sy.astype(mx.float32).reshape(-1, 1) / max(H - 1, 1)
    sp = mx.concatenate([px, py], axis=1)
    return sy, sx, sp, Ny, Nx, Ny * Nx


@lru_cache(maxsize=32)
def _pairs(Ny: int, Nx: int, r: int) -> tuple[mx.array, mx.array]:
    """4-connected + diagonal neighbour pairs within Chebyshev radius r.

    Cached via lru_cache — the result depends only on grid dimensions
    and radius, so repeated calls for the same image size are free.
    """
    s, d = [], []
    for yi in range(Ny):
        for xi in range(Nx):
            ...
            for dy in range(-r, r + 1):
                for dx in range(-r, r + 1):
                    ...
                    if 0 <= ny < Ny and 0 <= nx < Nx:
                        s.append(i)
                        d.append(ny * Nx + nx)
    return mx.array(s, dtype=mx.int32), mx.array(d, dtype=mx.int32)
...
def _boundary_channel(
    gw, radius: int = 4, n_theta: int = 8
) -> tuple[mx.array, mx.array, mx.array]:
    """Pb-style texture boundary channel (Martin, Fowlkes & Malik 2004).

    For each pixel and each candidate orientation θ, texture descriptors
    pooled in the two half-disks (radius `radius`) on either side of the
    line through the pixel at angle θ are compared; boundary strength is
    the max contrast over θ, the normal is the winning orientation.
    Unlike a gradient of the pooled energy, this measures texture change
    *across* a candidate boundary directly, and the descriptor includes
    orientation content (doubled-angle resultants), so boundaries between
    iso-energy textures are also visible.

    Descriptors: per-scale orientation-pooled energy (S maps,
    z-normalised) + global (C, S) resultant maps.

    Returns (bmap, normal, bimod), each (H, W): bmap in [0, 1], normal
    in radians (π-periodic), bimod = 2nd-best/best contrast ratio in
    [0, 1] — high at junctions where no single boundary orientation
    dominates.
    """
    H, W = gw.H, gw.W
    ...
    descs: list[mx.array] = []
    maps = [E[s].sum(axis=0) for s in range(gw.scale_size)]
    total = maps[0]
    for m in maps[1:]:
        total = total + m
    eps = 0.01 * mx.maximum(total.mean(), 1e-12)
    for m in maps:
        # spectral-shape descriptors (gain-controlled, see _scale_energy)
        # so a smooth illumination gradient is not read as a boundary
        sh = m / (total + eps)
        descs.append((sh - sh.mean()) / mx.maximum(sh.std(), 1e-8))
    amps = E.sum(axis=0)  # (O, H, W)
    amps_n = amps / mx.maximum(amps.sum(axis=0, keepdims=True), 1e-12)
    descs.append((amps_n * mx.cos(2.0 * thetas).reshape(-1, 1, 1)).sum(axis=0))
    descs.append((amps_n * mx.sin(2.0 * thetas).reshape(-1, 1, 1)).sum(axis=0))
    padded = [
        mx.pad(m, [(radius, radius), (radius, radius)], mode="edge") for m in descs
    ]
    ...
    d_maps: list[mx.array] = []
    for k in range(n_theta):
        th = k * math.pi / n_theta
        c, s = math.cos(th), math.sin(th)
        left = [(dy, dx) for dy, dx in coords if dx * c + dy * s < -0.5]
        right = [(dy, dx) for dy, dx in coords if dx * c + dy * s > 0.5]
        acc = mx.zeros((H, W), dtype=mx.float32)
        for p in padded:
            ml = sum(
                p[dy + radius : dy + radius + H, dx + radius : dx + radius + W]
                for dy, dx in left
            ) / len(left)
            mr = sum(
                p[dy + radius : dy + radius + H, dx + radius : dx + radius + W]
            ...
            acc = acc + (ml - mr) ** 2
        d_maps.append(mx.sqrt(acc))

    D = mx.stack(d_maps, axis=0)  # (n_theta, H, W)
    top2 = mx.sort(D, axis=0)[-2:]
    bmap = top2[1]
    normal = mx.argmax(D, axis=0).astype(mx.float32) * (math.pi / n_theta)
    # bimodality: ratio of 2nd-best to best contrast. ~1 at junctions
    # (no single dominant boundary orientation), low on clean boundaries
    bimod = top2[0] / mx.maximum(top2[1], 1e-12)
    bmax = mx.max(bmap)
    if float(bmax.item()) > 1e-12:
        bmap = bmap / bmax
    mx.eval(bmap, normal, bimod)
    return bmap, normal, bimod
