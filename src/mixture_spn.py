...
class MixtureSPN:
    ...
    def fit(
        ...
        basis_dim: int | None = None,  # 白化基内在维截断 (None 或 <=0 = 全维)
    ) -> MixtureSPN:
        """逐分层实例级组装 (P(stratum)·P(f,t,场景因子|stratum)), 确定性。

        basis_dim 保留最高方差的 basis_dim 维 (§10.3: 内在维 ~32–64, 截掉
        白化放大的低方差尾维噪声 → 模型 15× 更小 + 精度反升)。None 或 <=0
        表全维 (不截断), >=1 截断到 min(basis_dim, D)。与
        model_memory.truncate_basis 等价 (拟合期 vs 后置)。
        """
        f_mean, basis, z = cls._whiten(f)
        if basis_dim is not None and basis_dim >= 1:
