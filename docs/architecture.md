...
1. **熟悉尺寸先验重接** (prior.md 遗产): 单目 s/z 歧义的唯一合法
   解法 (实例模型下 log_w += log p(s) 重加权)。注: 立体模式下
   s/z 已被视差几何破解 (§6), 此先验只剩单目场景的意义
2. **局部线性核回归**: 治外推边界饱和 (责任度加权的局部线性拟合
   在边界外沿线性外推; z/s 在立体模式已由几何解决, 此项主要
   针对单目与其他目标)
...
偏差, 残差学习标定); 融合后 z R² 0.85 (单目 −0.12) / s R² 0.38
(−0.45) / **外推 z R² 0.96** (几何量不饱和, 核回归边界问题在 z/s
上消解); kind/u/v/hue 与单目持平。
