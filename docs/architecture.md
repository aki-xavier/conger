...
> 设计与主管线流程，与下方 §0–§11 的「全系统（内核 + 视觉）机制决策录」互补：
> §0–§11 记录视觉主线与历史机制演进，本节聚焦内核自身的分层架构、数据流、
...
flowchart TD
    ...
    subgraph L2["② 通用结构学习框架"]
        B["structured_hypothesis.v<br/>StructuredHypothesis (scene=voidptr 不透明载荷)"]
        C["generic_structure_gate.v<br/>GenericStructureGate: decide / decide_hierarchical"]
        D["generic_expert_registry.v<br/>GenericExpertRegistry + GenericExpert 接口"]
    ...
    subgraph L5["⑤ 工具"]
        J["types.v (MetaValue) · vecmath.v (f64 原语+RNG) · mlxutil.v (MLX 辅助)"]
    end
    subgraph L6["⑥ 非视觉验证域"]
...
- **不透明载荷**：`StructuredHypothesis.scene` 为 `voidptr`，由视觉层装箱 `cga.Scene`、内核不解箱
  —— 这是「内核不依赖视觉」的关键边界。`ForwardModel` 的观测参数同样是 `voidptr`。
- **接口协议**：`GenericExpert.estimate(observation mlx.Array) → StructuredHypothesis`、
...
| 结构框架 | `structured_hypothesis.v` | `StructuredHypothesis` · `HypothesisCandidate` · `new_hypothesis` · `factor_marginals` |
| | `generic_structure_gate.v` | `GenericStructureGate` · `GenericStructureDecision` · `softmax_map` · `decide` · `decide_hierarchical` |
| | `generic_expert_registry.v` | `GenericExpert` · `GenericExpertRegistry` |
...
| 工具 | `types.v` | `MetaValue` · `meta_str`/`meta_int`/`meta_f64`/`meta_map`/`meta_list` |
...
### 7.1 场景级几何-光照 ECM 精炼 (已实现, 下一步①验收通过)

**状态**: 已按通用 EM 框架 (`generic_em.EMLoop` + `scene_em_refiner.
SceneEMRefiner`) 实现, 接入单物体推理/评估链路 (默认关闭, `--em-refine`
开)。四维全搜 (u,v,s,z) 的全量 N=1296 验收未通过 —— u/v 提升但 s 崩溃;
按下一步①冻结 s/z (只精炼 u/v) 后全量验收**通过** (见下)。冻结 s/z 已
成为默认 (`em_freeze_sz=True`), `--em-no-freeze-sz` 回退到旧四维全搜。
...
**全量验收 (冻结 s/z, `em_freeze_sz=True` 默认, 2026-08-17)**:
插值 u/v RMSE 5.9/5.1 → 1.88/2.11 px (R² 0.930/0.945 → 0.990/0.987),
外推 R² 0.949/0.953 → 0.978/0.981; **s R² 不再崩溃** (插值 0.508→
0.516, 外推 0.922→0.933), z R² 反升 (0.831→0.864 / 0.956→0.962); kind
0.753→0.769 / 0.617→0.691, ldir 0.895→0.914, hue/lcol 不变 (1.000/
0.994)。self_check 通过 ✓。冻结 s/z 不仅规避了 s 崩溃, 还让 u/v 收敛
更干净 (坐标搜索不被退化的 s/z 方向分散) —— 插值 u/v R² 甚至超过四维
全搜的 0.963/0.971。

**成本**: 单样本推理 2.22s→4.58s (2.06×); 全量 run 基线 ~40min →
ECM ~80min。

**下一步状态** (按价值): ① s 不参与坐标搜索 (只精炼 u/v) **已落地** —
`em_freeze_sz=True` 默认, 全量验收通过; ② s/z 联合搜索 (而非独立 ±δ)、
③ 缩小 s 步长或加约束 留待需要恢复 s/z 精炼时再评。ECM 仍默认关闭
(推理 2.06×, 全量 ~80min; 冻结 s/z 后 M 步少一半几何候选, 成本略降);
但单物体指标已不降反升, 可按需 `--em-refine` 开启。双层仍未接 (逐层
几何稳定后再说)。
...
### 10.6 多物体族基截断 (layered/composite, 2026-08-17)

§10.3 的单物体结论 (D≈48 全维度赢) 是否泛化到多物体? `basis_sweep.py`
后置截断扫描 + 全量验收 (全维 D≈1360):

**composite (附着组合, 结构化双物体)**: D=48 是**干净赢面** —— 全量验收
插值 kind0 0.511→0.527 / kind1 0.355→0.371 / hue0 0.504→0.714 / hue1
0.365→0.524 / lcol 0.398→0.451 / u0 R² 0.982→0.991 / s1 R² -0.474→0.115
(转正) / z1 0.875→0.922; 外推同样不退化 (kind0 0.489 / hue0 0.610)。
模型 1.27GB→46MB (**27×**)。

**layered (独立前后层遮挡, 非结构化)**: D=48 是**插值赢、外推离散退化**
的折中 —— 插值 kind0 0.398→0.599 / hue0 0.421→0.679 / hue1 0.171→0.333
/ u0 R² 0.537→0.618 / v0 0.711→0.810 全升; 但**外推离散因子退化到近随机**
(kind0 0.407→0.333 / hue0 0.359→0.167 / lcol 0.382→0.333)。后层 s/z
(s1 R² -15.4 / z1 -4.8) 全维即已断裂 (`RESIDUAL_SCALE` 后层 s/z 残差
归零的设计), 与 D 无关。

**根因 (与单物体的差异)**: 单物体/结构化组合的类别信号集中在高方差维,
截断去噪即赢; 但独立遮挡的**后层外观信号被遮挡压到低方差尾维**, 且外推
时陌生几何淹没外观线索 → 截断在插值去掉噪声、在外推丢掉后层外观信号。
这是 §2.1 零空间抖动与遮挡纠缠的叠加: 结构化 (composite) 信号集中 →
截断赢; 非结构化遮挡 (layered) 信号分散 → 截断是折中。

**落地**: basis_dim=48 全局默认对 composite 保持; layered 属实验族 (无
渲染精炼、后层 s/z 未接), 插值收益大、外推离散退化已记录。若需 layered
外推不退化, 单独 `--basis-dim 128` 重训。

## 11. 四机制同源: 白化基是枢纽 (2026-08-17)

增量训练 (§8)、按需加载 (§10.2)、动态遗忘 (§10.3/10.4) 与「不做 PCA
白化」看似四个独立机制, 实则是**同一个对象 (白化基) 在不同轴上的经济
手段**。枢纽是 PCA 白化基 `basis (V×D)`:

- **买来正确性**: §2.1 实测, 不做白化 → 对角高斯把相关像素当独立选票,
  kind 0.47 (1-NN 0.95 → 数据无罪、模型有病); 白化让「对角高斯 ≡ 原空间
  全协方差」, 这是实例级混合能用的前提。
- **付出代价**: 基占 **98.7% 内存** (453.5/459.6MB), 拟合 O(N³) Gram eigh,
  且分布漂移要重算。

所以「不做 PCA 白化」**不是可选项** (§2.1 已否)。真正的根问题是: 能否
让「去相关」比全量数据驱动 PCA 更便宜。另三个机制都是在「必须要有基」
的前提下, 对基的代价做减法:

| 机制 | 作用轴 | 对基做什么 |
|---|---|---|
| 增量训练 `add` | 时间轴 | 冻结基 (避免重复 eigh), 只追加分量 |
| 按需加载 `split/load` | 空间轴 | 分级物化 (白化时才载基) |
| 动态遗忘 `truncate/forget` | 维度+分量轴 | D↓ 截基、K↓ 驱逐 |
| 不做白化 | 根 | 取消这个对象 |

**成对关联**:

1. **增量训练 ⇄ 按需加载**: `add` 冻结基 → 基在跨追加时是稳定、可分离的
   静态对象。正是这个冻结让 `split_save` 把基单独拆出、`load_components`
   只载 6MB 分量表而不载 454MB 基成为可能。**冻结 (时间复用) 是分离
   (空间复用) 的前提。**
2. **增量训练 ⇄ 动态遗忘**: K 轴两者方向相反 —— 增量「只记不忘」(K 无限
   追加), 遗忘「驱逐」(K↓), 是同一根内存上界的增长与回收; N=1296 实测
   驱逐 K 有害 (kind 0.781→0.547), 故「只记不忘」当前是对的。D 轴则是
   **还债**: 增量训练把基冻结在过完备 D=497, 截断发现内在维仅 ~32–64 ——
   截断偿还的是增量训练冻结时一并冻结下来的冗余 (增量不做维度选择,
   遗忘补上)。
3. **按需加载 ⇄ 动态遗忘**: 两者解决「基太大」的不同阶段。截断直接缩小
   「要载的那个东西」(459→30MB), 部分覆盖按需加载的边际收益 —— 这是
   「先 D 截断」的深层原因: 先缩小它, 再谈分时载它。
4. **不做白化 ⇄ 三者**: 根问题, 一荣俱荣。找到比全量 PCA 更便宜的去相关
   (固定正交 / 低秩+对角 / 子样本估基) → 基变小, 按需加载与遗忘都不再
   需要; 反过来 `truncate_basis` 正是「不做全量白化」的务实折中 (保留去
   相关能力, 只留内在维), 已把 §7.8 逃生通道的「基按内在维截断 / 子样本
   估基全量套用」先走了一条。

**贯穿规律 (与因果发现同构的不对称)**: 因果「反照率对边缘化鲁棒、光照
不鲁棒」, 遗忘「D 截断是赢、K 驱逐是亏」—— 同源: K (样本密度) 与
D (内在维) 当前处在支撑集不同侧。K=1296 还「不够密」, 删样本就丢信息;
D=497 却「过密」, 删维度反而去噪。**动态遗忘不能一概而论, 必须拆 D/K
两轴分别判定。**

**落地含义**: ① 最优单步 = D 截断 (一个动作同时缓解内存/增量过完备/基
太大, 精度不降反升); ② K 驱逐暂缓 (与增量「只记不忘」一致, 等 N 上量级);
③ 「不做白化」降级为研究问题 (找比全量 PCA 更便宜的去相关), 不阻塞
D 截断这个已验证落点。
