...
1. **组合模板联合门控**: cp2 已完成全量训练, 下一步与 single/layered
...
### 7.1 TODO: 场景级几何-光照 ECM 精炼 (暂不做)

**状态**: 仅记录设计, 暂不实现。目标是推理阶段的分析-合成迭代,
不是恢复 §2.2 已退役的 SPN 学习/质心压缩 EM。

**数学形式**: 场景拆成几何 \(G\) 与外观/光照 \(A\)。固定 \(G_t\)
时枚举或取 top-k 外观候选 \(A_j\), 用 cga 渲染残差形成后验

```math
q_t(A_j)\propto p(A_j)\exp(-\ell(I,R(G_t,A_j))/T)
```

固定 \(q_t\) 时做几何局部最大化:

```math
G_{t+1}=\arg\min_G\sum_j q_t(A_j)\ell(I,R(G,A_j))
```
...
**成本与风险**: 单物体一轮 54×2 渲染, EM 2 轮加几何候选约 2–5×;
双层若不做 top-k/局部搜索会立刻不可行。风险是外观歧义过早塌缩、
温度过自信、局部最优、遮挡顺序翻转和后层幻觉。验收条件: 合成样本
期望残差不增; 单物体指标至少不降且成本可接受; 双层只在逐层几何
稳定后进入。
