...
fn em_render_true(cb Codebook, mut renderer cga.Renderer, cam_l cga.PerspectiveCamera, cam_r cga.PerspectiveCamera, true_geom []f64) (mlx.Array, mlx.Array) {
	prm := [0.0, true_geom[0], true_geom[1], true_geom[2], true_geom[3], 2.0, 0.0, 1.0]
	sc := cb.to_scene(prm)
...
fn test_scene_em_freezes_sz_by_default() {
	...
	uv_err := math.sqrt((got[0] - true_geom[0]) * (got[0] - true_geom[0]) +
		(got[1] - true_geom[1]) * (got[1] - true_geom[1]))
...
fn new_quadratic_refiner(freeze []bool) SceneEMRefiner {
	mut r := new_scene_em_refiner(new_codebook(InverseConfig{}), 0, mlx.zeros([8, 8, 3], .float32), mlx.zeros([
		8,
		8,
		3,
	], .float32), 1, freeze)
