...
本仓库是「SPN 逆渲染研究」拆分为两个平行项目后的 **通用内核**：MixtureSPN（全分辨率实例级浅混合 SPN：PCA 白化 + 逐 kind 分层，每样本一个对角高斯块，连续条件期望 ≡ 分层核回归，离散场景因子 ≡ 条件后验分类，无 EM、确定性）+ 通用结构学习框架（`StructuredHypothesis` / `EMLoop` / `GenericStructureGate` / `GenericExpertRegistry` / `StructureBirthController` / `KernelGraph` 似然核网络）+ 模板学习（血缘 / 文法 / 约束学习）+ 模型内存与持久化 + 纯数学 / MLX 工具 + 非视觉验证域（`ToySeries`）。内核不依赖图像、`cga` 渲染或任何视觉概念。
...
本仓库是 **V 语言移植版**: 模块 `conger` 平铺在仓库根目录，依赖经默认 V 模块目录 `~/.vmodules` 解析（符号链接到本地 [`mlx-v`](../mlx-v) 绑定）。内核架构与主管线的详细说明见 `docs/architecture.md`；视觉层的机制决策见 [`conger-vision/docs/architecture.md`](../conger-vision/docs/architecture.md)。
...
依赖方向单向：核心 SPN → 结构框架 → 模板学习 → 持久化；工具层（⑤）被其余各层复用；验证域与示例（⑥）以 `ToySeries` 时间序列实例驱动核心 SPN、结构门控与出生控制，证明整套通用结构学习框架可脱离视觉独立运行（视觉路径只是它的一个领域适配器）。
...
非视觉验证：`toy_series_family.v` + `toy_series_expert.v`（线性 / 振荡时间序列专家，已导出为 pub，验证 MixtureSPN、结构门控与出生控制不依赖图像或 cga）。

测试：根目录 `*_test.v`（11 个文件）。`docs/architecture.md` — 内核架构与主管线（分层架构 + 数据/控制流总图）；视觉层机制决策见 `conger-vision/docs/architecture.md`。
