...
测试：根目录 `*_test.v`（15 个文件）。`docs/architecture.md` — 内核架构与主管线（分层架构 + 数据/控制流总图）。
...
示例的可视化为自包含 plotly.js HTML（经标准库 `json2` 生成 trace/layout JSON，CDN 加载 plotly），无第三方模块依赖。
