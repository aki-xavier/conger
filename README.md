...
视觉项目 `conger-vision` 通过 `import conger` 使用本内核：`conger.fit_mixture_spn` / `conger.load_mixture_spn` 训练与加载模型，`net.predict(f)` 返回 `(E[t|x], P(scene factors|x), 责任度)`；`conger.StructuredHypothesis` 承载 MAP 场景（`scene` 载荷）、候选联合后验与新颖性证据；`conger.GenericStructureGate` / `conger.GenericExpertRegistry` 组合门控与出生控制。
